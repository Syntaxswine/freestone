...
const DROWNED = new THREE.Color(0x3d5f78); // wash for rock below the water table
...
export class UnderworldLayer {
  ...
  /** the depth ruler: fathoms below the crown, the seam or rock, and whether the
   *  ground at the site centre is drowned (below the water table — needs drainage) */
  readout(): {
    fathoms: number;
    elevationAOD: number;
    label: string | null;
    material: GroundMaterial | 'open air';
    drowned: boolean;
    waterTableAOD: number;
  } {
    ...
    return {
      ...
      material,
      drowned: material !== 'open air' && this.elevation < wt,
      waterTableAOD: wt,
    };
  }
}
