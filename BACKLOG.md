...
freeing hands; horse rendered ON the road w/ a sledge-load) → 42 (THE WAY-WORTH READOUT — a causeway shows whether the
ground is worth a road as you draw it; the readout CAUGHT that the way was near-inert on the real Durham map,
WAY_DRY_DEPTH re-anchored to the evidence's ordinary-earth ~3×).** 229 tests; canon re-authored (wall A the carry specimen; ids
...
(boss-ruled, build-ready). SHIPPED: **SIM 45 LINEAGE (a child inherits a head-start in a parent's finest trade → dynasties) · **SIM 43 the SKILL LADDER** (green→journeyman→master, ×9/8→×5/4→×3/2) ·
